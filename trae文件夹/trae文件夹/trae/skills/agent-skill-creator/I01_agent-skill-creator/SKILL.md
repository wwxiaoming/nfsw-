...
description: "从工作流描述创建跨平台代理技能。当用户要求创建代理、自动化重复工作流、创建自定义技能或需要高级代理创建时激活。触发词包括：create agent for、automate workflow、create skill for、every day I have to、daily I need to、turn process into agent、need to automate、create a cross-platform skill、validate this skill、export this skill、migrate this skill。支持单一技能、多代理套件、转录处理、基于模板的创建。"
